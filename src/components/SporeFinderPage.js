...
const SporeFinderPage = () => {
  ...
  const [filteredSpores, setFilteredSpores] = useState([]);
  ...
  useEffect(() => {
    LoggingService.debug('SporeFinderPage: Filtering spores', {
    ...
    let filtered = [...sporeData];
    
    // Apply type filter
    if (filterType !== 'all') {
      filtered = filtered.filter(spore => spore.type === filterType);
    }
    
    // Apply search filter
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter(spore => {
        return (
          spore.name.toLowerCase().includes(term) ||
          spore.description.toLowerCase().includes(term) ||
          spore.scientificName.toLowerCase().includes(term)
        );
      });
    }
    
    setFilteredSpores(filtered);
  }, [searchTerm, filterType, sporeData, sporeData.length, error]);
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-0 md:px-4 py-4 md:py-8">
        <div className="w-full card bg-card-bg p-4 md:p-6 rounded-none md:rounded-2xl shadow-md h-full">
          <h1 className="text-2xl font-bold mb-4 md:mb-6">Spore Finder</h1>
          <div className="animate-pulse space-y-4 md:space-y-6 py-4 md:py-6">
            <div className="h-4 bg-slate-200 rounded w-3/4"></div>
            <div className="h-10 bg-slate-200 rounded w-full"></div>
            <div className="grid grid-cols-1 gap-0 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-white rounded-none md:rounded-xl shadow-sm p-4 md:p-6 border-b border-gray-200 md:border md:border-gray-100 mb-2 md:mb-0">
  ...
  if (error) {
    return (
      <div className="container mx-auto px-0 md:px-4 py-4 md:py-8">
        <div className="w-full card bg-card-bg p-4 md:p-6 rounded-none md:rounded-2xl shadow-md h-full text-center">
          <h1 className="text-2xl font-bold mb-4 md:mb-6">Spore Finder</h1>
  ...
  return (
    <div className="container mx-auto px-0 md:px-4 py-4 md:py-8">
      <div className="w-full card bg-card-bg p-4 md:p-6 rounded-none md:rounded-2xl shadow-md h-full">
        ...
        {/* Spore Cards - Mobile optimized grid */}
        <div className="grid grid-cols-1 gap-0 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredSpores.map(spore => (
            <div key={spore.id} className="bg-white rounded-none md:rounded-xl shadow-sm p-4 md:p-6 border-b border-gray-200 md:border md:border-gray-100 mb-2 md:mb-0 hover:shadow-md transition-shadow">
              <div className="flex items-center mb-4">
